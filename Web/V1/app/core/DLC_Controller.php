...
/*
$sqls = $this->db->insert_batch($table, $data, null, 100, false);
foreach($sqls as $sql){
...
class SM_Controller extends CI_Controller { 
    ...
    public function __construct() {
		...
		if(empty($this->_role[3]))$this->_role[3] = array(0);
        include  "yun/gfun.php";
    }
    
    public function assign($key,$val) {
    	
        $this->ci_smarty->assign($key,translate($val));
    }
	...
	//翻译
    public function translate($language,$data){
        if($language==1){
            $where_field="japanese";
            $get_field="chinese";
        }else{
            $where_field="chinese";
            $get_field="japanese";
        ...
        if(is_array($data)){
            foreach($data as $k=>$v){
                $data[$k]=$this->translate($language,$v);
            }
            ...
            $str=$this->_getfield('language',$get_field,[$where_field=>$data]);
            if($str) $data=$str;
        }
        return $data;
    }
	...
	public function _rolesql($alias = '', $field = 'adminid', $adminid = 0){		
		...
		if(in_array($gid, $this->_role[1]) || in_array($gid, $this->_role[2])){
			$row = $this->_rolerow($aid, $gid);
			$aids = $row['_allchild'];
			if(!$aids)$aids = $aid;
			return " and ".($alias ? $alias.'.' : '').$field." in (".$aids.") ";
		}elseif(in_array($gid,$this->_role[3])){//员工 子账号
		    $parent_id=$this->_getfield('staff','parent_id',['aid'=>$this->_admin->id]);
		    if($parent_id==1) return '';
		    $parent=$this->_getrow('admin','id,gid',['id'=>$parent_id]);
            $row = $this->_rolerow($parent['id'], $parent['gid']);
            $aids = $row['_allchild'];
            if(!$aids)$aids = $aid;
            return " and ".($alias ? $alias.'.' : '').$field." in (".$aids.") ";
        }
	...
	//获取权限字段
    public function _getrolefield($field=[]){
        if($this->_admin->_role_type==1){
            return $field[1]?$field[1]:"agent_id";
        }elseif($this->_admin->_role_type==2){
            return $field[2]?$field[2]:"adminid";
        }elseif($this->_admin->_role_type==3){
            $role_type=$this->_getfield('staff','type',['aid'=>$this->_admin->id]);
            if($role_type==1){
                return $field[3]?$field[3]:"agent_id";
            }else{
                return $field[4]?$field[4]:"adminid";
            }
        }
        return '';
    }
     ...
     * @param array $data
     */
    public function admin_return($status=0,$msg='',$data=[]){
        die(json_encode(['status'=>$status,'msg'=>translate($msg),'data'=>translate($data)]));
    }

    /** 判断时间是否是 0000-00-00 00:00:00 如果是 就返回 空字符串 lzj
     * @param $data
     * @return mixed
     */
    public function replace_null_time($data,$type=true){
        if(is_array($data)){
            foreach($data as $k=>$v){
                if($v=='0000-00-00 00:00:00') $data[$k]='';
                if($type) if($v=='0000-00-00') $data[$k]='';
            }
        }
        return $data;
    }
     ...
     * @return string X小时X分钟 || X分钟
     */
    public function calculate_time($start_time=0,$end_time=0,$minute=''){
        if(!is_numeric($minute)){
             if($end_time=="0000-00-00 00:00:00" || !$end_time || $end_time<0) $end_time=time();
            //计算使用时长
            $minute=floor(( $end_time-$start_time)/60);//分钟
        }
        $time_arr=explode('.',(round($minute/60,2)));
        if($time_arr[0]){
            $h=$time_arr[0]."小时";
        }
        $str=substr((round($minute/60,2)*100),-2);
        $minute=floor($str/100*60);
        if($minute>0){
            $minute=$minute."分钟";
        }
        $str=$h.$minute;
	...
	public function _message($msg, $goto = '', $auto = TRUE, $fix = '', $pause = 3000)
	{
		...
		$goto .= $fix;
		 */
		$data = array('msg' => translate($msg), 'goto' => $goto, 'auto' => $auto, 'pause' => $pause);
	...
	public function _getpage($table, $where, $order = false, $page = 1, $page_size = 20, $field = '*'){
		...
		$this->pagination->init($total_rows,'',$page_size);
    ...
    //重启设备
    public function _reboot($macno){
        $redis_beat=gfun::get_redis($macno.":COM");
        $macinfo=json_decode($redis_beat,1);
        if($macinfo['lastbeat']+100<time()) return array("code"=>-1,"msg"=>"设备不在线");
        if(!empty($macinfo)){
            $cmd = "80FF";
            $command = "3AA300000001".substr("0000000000000000".$macno,-16).substr("000".dechex(strlen($cmd)/2),-4).$cmd;
            log_message("error","_reboot".$command);
            $sendcom=gfun::hextostr($command.gfun::getcrc16($command));
    ...
    //停止运行
    public function _macStop($macno){
        $redis_beat=gfun::get_redis($macno.":COM");
        $macinfo=json_decode($redis_beat,1);
        if($macinfo['lastbeat']+100<time()) return array("code"=>-1,"msg"=>"设备不在线");
        $sendtime=time();
        if(!empty($macinfo)){
            $command = "3AA300000001".substr("0000000000000000".$macno,-16)."000209D1";
            log_message("error","_macStop".$command);
            $sendcom=gfun::hextostr($command.gfun::getcrc16($command));
            if($this->send($macinfo['ip'],$macinfo['port'],$sendcom)){
                $ret["code"]=1;
                $ret["msg"]="发送成功";
                if(empty($seconds)) $seconds=5;
                $seconds=$seconds*10;
                while($seconds/10>0){
                    usleep(100000);
                    $redis_ret=json_decode(gfun::get_redis($macno.":09D1"),1);
                    if(!empty($redis_ret)&&$redis_ret['ctime']>=$sendtime){
                        $ret['data'] = $redis_ret;
                        gfun::delete_redis($macno.":09D1");
                        break;
                    }
                    $seconds--;
                }
        ...
        //$ret['data']['status']//交易结果:0：清空交易成功，表示已接收到命令并正确处理；3：清空交易失败，设备故障;16:设置失败，指令错误
        //$ret['data']['remain']//剩余使用次数
        log_message("error",$macno."-->".json_encode($ret));
        return $ret;
    }
    //直接运行
    //sysnum 系统流水 用于识别确认，可传当前时间
    //times 使用次数
    public function _macRun($macno,$sysnum,$times){
        $redis_beat=gfun::get_redis($macno.":COM");
        $macinfo=json_decode($redis_beat,1);
        if($macinfo['lastbeat']+100<time()) return array("code"=>-1,"msg"=>"设备不在线");
        if(!empty($macinfo)){
            $cmd = "09D0".substr("0000000".dechex($sysnum),-8).substr("000".dechex($times),-4);
            $command = "3AA300000001".substr("0000000000000000".$macno,-16).substr("000".dechex(strlen($cmd)/2),-4).$cmd;
            log_message("error","_macRun".$command);
            $sendcom=gfun::hextostr($command.gfun::getcrc16($command));
            if($this->send($macinfo['ip'],$macinfo['port'],$sendcom)){
                $ret["code"]=1;
                $ret["msg"]="发送成功";
                if(empty($seconds)) $seconds=10;
                $seconds=$seconds*10;
                while($seconds/10>0){
                    usleep(100000);
                    $redis_ret=json_decode(gfun::get_redis($macno.":09D0"),1);
                    if(!empty($redis_ret)&&$redis_ret['sysnum']>=$sysnum){
                        $ret['data'] = $redis_ret;
                        gfun::delete_redis($macno.":09D0");
                        break;
                    }
                    $seconds--;
                }
        ...
        log_message("error",$macno."-->".$sysnum."----".json_encode($ret));
        return $ret;
    }
    //读取参数
    public function _getParam($macno){
        $redis_beat=gfun::get_redis($macno.":COM");
        $macinfo=json_decode($redis_beat,1);
        if($macinfo['lastbeat']+100<time()) return array("code"=>-1,"msg"=>"设备不在线");
        $sendtime=time();
        if(!empty($macinfo)){
            $command = "3AA300000001".substr("0000000000000000".$macno,-16)."00028085";
            log_message("error","_getParam".$command);
            $sendcom=gfun::hextostr($command.gfun::getcrc16($command));
            if($this->send($macinfo['ip'],$macinfo['port'],$sendcom)){
                $ret["code"]=1;
                $ret["msg"]="发送成功";
                ...
                while($seconds/10>0){
                    usleep(100000);
                    $redis_ret=json_decode(gfun::get_redis($macno.":8085"),1);
                    if(!empty($redis_ret)&&$redis_ret['ctime']>=$sendtime){
                        $ret['data'] = $redis_ret['data'];
                        gfun::delete_redis($macno.":8085");
                        break;
                    }
                    $seconds--;
                }
        ...
        //$ret['data']['result']//0表示成功
        //$ret['data']['rate']//波特率
        //$ret['data']['macType']//设备类型
        log_message("error",$macno."-->".json_encode($ret));
        return $ret;
    }
    //设置参数
    //rate 波特率
    //macType 设备类型
    public function _setParam($macno,$rate,$macType){
        $redis_beat=gfun::get_redis($macno.":COM");
        $macinfo=json_decode($redis_beat,1);
        if($macinfo['lastbeat']+100<time()) return array("code"=>-1,"msg"=>"设备不在线");
        if(!empty($macinfo)){
            $cmd = "8005".substr('00000000'.dechex($rate),-8).substr('00'.dechex($macType),-2);
            $command = "3AA300000001".substr("0000000000000000".$macno,-16).substr("000".dechex(strlen($cmd)/2),-4).$cmd;
            log_message("error","_setParam".$command);
            $sendcom=gfun::hextostr($command.gfun::getcrc16($command));
            $sendtime=time();
            if($this->send($macinfo['ip'],$macinfo['port'],$sendcom)){
                $ret["code"]=1;
                ...
                while($seconds/10>0){
                    usleep(100000);
                    $redis_ret=json_decode(gfun::get_redis($macno.":8005"),1);
                    if(!empty($redis_ret)&&$redis_ret['ctime']>=$sendtime){
                        $ret['data'] = $redis_ret['data'];
                        gfun::delete_redis($macno.":8005");
                        break;
                    }
                    $seconds--;
                }
    ...
    public function _getAPN($macno){
        ...
        if(!empty($macinfo)){
            $cmd = "8086";
            $command = "3AA300000001".substr("0000000000000000".$macno,-16).substr("000".dechex(strlen($cmd)/2),-4).$cmd;
            log_message("error","_getAPN".$command);
            $sendcom=gfun::hextostr($command.gfun::getcrc16($command));
            $sendtime=time();
            if($this->send($macinfo['ip'],$macinfo['port'],$sendcom)){
                $ret["code"]=1;
                ...
                while($seconds/10>0){
                    usleep(100000);
                    $redis_ret=json_decode(gfun::get_redis($macno.":8086"),1);
                    if(!empty($redis_ret)&&$redis_ret['ctime']>=$sendtime){
                        $ret['data'] = $redis_ret['data'];
                        gfun::delete_redis($macno.":8086");
                        break;
                    }
                    $seconds--;
                }
    ...
    //设置APN
    //apn APN
    public function _setAPN($macno,$apn){
        $redis_beat=gfun::get_redis($macno.":COM");
        $macinfo=json_decode($redis_beat,1);
        if($macinfo['lastbeat']+100<time()) return array("code"=>-1,"msg"=>"设备不在线");
        if(!empty($macinfo)){
            $cmd = "8006".substr('0000'.dechex(strlen($apn)),-4).substr(bin2hex($apn),-64);
            $command = "3AA300000001".substr("0000000000000000".$macno,-16).substr("000".dechex(strlen($cmd)/2),-4).$cmd;
            log_message("error","_setAPN".$command);
            $sendcom=gfun::hextostr($command.gfun::getcrc16($command));
            $sendtime=time();
            if($this->send($macinfo['ip'],$macinfo['port'],$sendcom)){
                $ret["code"]=1;
                ...
                while($seconds/10>0){
                    usleep(100000);
                    $redis_ret=json_decode(gfun::get_redis($macno.":8006"),1);
                    if(!empty($redis_ret)&&$redis_ret['ctime']>=$sendtime){
                        $ret['data'] = $redis_ret['data'];
                        gfun::delete_redis($macno.":8006");
                        break;
                    }
                    $seconds--;
                }
    ...
    //获取用户名
    public function _getUserName($macno){
        $redis_beat=gfun::get_redis($macno.":COM");
        $macinfo=json_decode($redis_beat,1);
        if($macinfo['lastbeat']+100<time()) return array("code"=>-1,"msg"=>"设备不在线");
        if(!empty($macinfo)){
            $cmd = "8087";
            $command = "3AA300000001".substr("0000000000000000".$macno,-16).substr("000".dechex(strlen($cmd)/2),-4).$cmd;
            log_message("error","_getUserName".$command);
            $sendcom=gfun::hextostr($command.gfun::getcrc16($command));
            $sendtime=time();
            if($this->send($macinfo['ip'],$macinfo['port'],$sendcom)){
                $ret["code"]=1;
                ...
                while($seconds/10>0){
                    usleep(100000);
                    $redis_ret=json_decode(gfun::get_redis($macno.":8087"),1);
                    if(!empty($redis_ret)&&$redis_ret['ctime']>=$sendtime){
                        $ret['data'] = $redis_ret['data'];
                        gfun::delete_redis($macno.":8087");
                        break;
                    }
                    $seconds--;
                }
    ...
    //设置用户名
    //userName 用户名
    public function _setUserName($macno,$userName){
        $redis_beat=gfun::get_redis($macno.":COM");
        $macinfo=json_decode($redis_beat,1);
        if($macinfo['lastbeat']+100<time()) return array("code"=>-1,"msg"=>"设备不在线");
        if(!empty($macinfo)){
            $cmd = "8007".substr('0000'.dechex(strlen($userName)),-4).substr(bin2hex($userName),-64);
            $command = "3AA300000001".substr("0000000000000000".$macno,-16).substr("000".dechex(strlen($cmd)/2),-4).$cmd;
            log_message("error","_setUserName".$command);
            $sendcom=gfun::hextostr($command.gfun::getcrc16($command));
            $sendtime=time();
            if($this->send($macinfo['ip'],$macinfo['port'],$sendcom)){
                $ret["code"]=1;
                ...
                while($seconds/10>0){
                    usleep(100000);
                    $redis_ret=json_decode(gfun::get_redis($macno.":8007"),1);
                    if(!empty($redis_ret)&&$redis_ret['ctime']>=$sendtime){
                        $ret['data'] = $redis_ret['data'];
                        gfun::delete_redis($macno.":8007");
                        break;
                    }
                    $seconds--;
                }
    ...
    //获取密码
    public function _getPassword($macno){
        $redis_beat=gfun::get_redis($macno.":COM");
        $macinfo=json_decode($redis_beat,1);
        if($macinfo['lastbeat']+100<time()) return array("code"=>-1,"msg"=>"设备不在线");
        if(!empty($macinfo)){
            $cmd = "8088";
            $command = "3AA300000001".substr("0000000000000000".$macno,-16).substr("000".dechex(strlen($cmd)/2),-4).$cmd;
            log_message("error","_getPassword".$command);
            $sendcom=gfun::hextostr($command.gfun::getcrc16($command));
            $sendtime=time();
            if($this->send($macinfo['ip'],$macinfo['port'],$sendcom)){
                $ret["code"]=1;
                ...
                while($seconds/10>0){
                    usleep(100000);
                    $redis_ret=json_decode(gfun::get_redis($macno.":8088"),1);
                    if(!empty($redis_ret)&&$redis_ret['ctime']>=$sendtime){
                        $ret['data'] = $redis_ret['data'];
                        gfun::delete_redis($macno.":8088");
                        break;
                    }
                    $seconds--;
                }
    ...
    //设置密码
    //password 密码
    public function _setPassword($macno,$password){
        $redis_beat=gfun::get_redis($macno.":COM");
        $macinfo=json_decode($redis_beat,1);
        if($macinfo['lastbeat']+100<time()) return array("code"=>-1,"msg"=>"设备不在线");
        if(!empty($macinfo)){
            $cmd = "8008".substr('0000'.dechex(strlen($password)),-4).substr(bin2hex($password),-64);
            $command = "3AA300000001".substr("0000000000000000".$macno,-16).substr("000".dechex(strlen($cmd)/2),-4).$cmd;
            log_message("error","_setPassword".$command);
            $sendcom=gfun::hextostr($command.gfun::getcrc16($command));
            $sendtime=time();
            if($this->send($macinfo['ip'],$macinfo['port'],$sendcom)){
                $ret["code"]=1;
                ...
                while($seconds/10>0){
                    usleep(100000);
                    $redis_ret=json_decode(gfun::get_redis($macno.":8008"),1);
                    if(!empty($redis_ret)&&$redis_ret['ctime']>=$sendtime){
                        $ret['data'] = $redis_ret['data'];
                        gfun::delete_redis($macno.":8008");
                        break;
                    }
                    $seconds--;
                }
    ...
    //设备是否在线
    public function mac_status($macno){
	$redis_beat=gfun::get_redis($macno.":COM");
	$macinfo=json_decode($redis_beat,1);
	if($macinfo['lastbeat']+30>time()) return 1;
	return 0;
        // $return=json_decode($ret,1);
        // if($return['code']==1&&isset($return['data']['status'])&&$return['data']['status']==0) return $return['data'];
        // return false;
    }
    private function send($ip,$port,$command){
		...
		$client->set(array(
				...
				'bind_port'     =>  $this->config->item('port'),
		));
    ...
    //微信 type ==1 正常退款  ==2 退超时订单
    public function wx_refund($trade,$money,$type=1){
        $trade_type = "JSAPI";//checknull($this->input->post('trade_type'), 'APP');
        // 调用微信扫码支付接口配置信息
        ...
        //由于此类库t构造函数需要传参，我们初始化类库就传参数给他吧
        $this->load->library('CI_Wechatpay', $wxconfig);

        if($type==1){
            $result = $this->ci_wechatpay->refund($trade['tradeno'], $trade['tradeno'],($trade['money']-$trade['timeout_money'])*100,$money*100, "SYS001");
        }else{
            $result = $this->ci_wechatpay->refund($trade['takes_no'], $trade['takes_no'],$trade['timeout_money']*100, $trade['timeout_money']*100, "SYS001");
        }
    ...
    //支付宝退款
    public function ali_refund($order,$type=1){
        ...
        $order_no=($type==1)?$order['tradeno']:$order['takes_no'];
        $trade_no="";
        $refund_money=($type==1)?($order['pay_money']-$order['timeout_money']):$order['timeout_money'];
        $out_request_no=time().rand(100,999);
        ...
        log_message('error',json_encode($result->$responseNode));
        //$this->db->insert('lzj_test',['desc'=>json_encode($resultCode)]);
        return $resultCode;

    }

    //发送短信
    public function ali_sms($TemplateCode="SMS_181851631",$Param=[],$area_code='86',$phone='',$SignName="雅尚科技"){
        include APPPATH.'libraries/aliyun-dysms-php-sdk-lite/SignatureHelper.php';
        $params = array ();
        $accessKeyId = $this->config->item('aliyun_sms_key');
        $accessKeySecret = $this->config->item('aliyun_sms_secret');
        // fixme 必填: 短信接收号码
        $params["PhoneNumbers"] = $area_code.$phone;
...
class Api_Controller extends SM_Controller {
    public $openid = 'oQy2IwujlnGtD7rHGnRKsrbO4iHA';
    public function __construct() {
        parent::__construct();
        /*if(isweixin()) $this->_check_login();
        if(is_alipay()){
                $this->_ali_check_login();
        }*/
    }
    protected function _check_login()
    {
        ...
        $this->load->library('weixin_api',$wxconfig);
        if (isset($_GET['code'])) {
            $access_token = $this->weixin_api->get_token_url($_GET['code']);
            $obj = json_decode(file_get_contents($access_token), 'true');
            if ($obj && isset($obj['openid']))
            {
                $this->openid = $obj['openid'];
                $this->input->set_cookie('openid', $this->openid, time()+3600*24);
                $info = $this->db->query("select id,weixin_openid from user where weixin_openid='" . $this->openid . "'")->row_array();
                /*if(!$info)
                {*/
                //获取用户基本信息
                $obj = json_decode(file_get_contents(sprintf('https://api.weixin.qq.com/sns/userinfo?access_token=%s&openid=%s&lang=zh_CN', $obj['access_token'], $obj['openid'])), 1);
                //var_export($aUserInfo);
                //$mysql->insert('wxusers',$aUserInfo);
                log_message('Error', __CLASS__ . ' ' . __FUNCTION__ . json_encode($obj));
                $wxuser = array (
                    'weixin_openid' => $this->openid,
                    'nickname'=>$obj['nickname'],
                    //							'sex'=>$obj['sex'],
                    'avatar'=>$obj['headimgurl'],
                    //							'location'=>"$obj->country"."$obj->province"."$obj->city",
                    'status' => 1,
                    'createtime'=>time()
                );
                if ($info['id'])
                    $this->db->update('user',$wxuser,array("id"=>$info['id']));
                else
                    $this->db->insert('user',$wxuser);
                /*}*/

            }
        }
        $user=$this->_getrow('user','id',['weixin_openid'=>$_COOKIE['openid']]);
        if(empty($_COOKIE['openid']) || !$user){
            $return_url = urlencode(sprintf('%s%s', base_url(), $_SERVER['REQUEST_URI']));
            $get_openid_url = $this->weixin_api->get_openid_url($return_url);
            header('Location:'.$get_openid_url);exit;
        }
        $this->openid = $_COOKIE["openid"];
    }
    protected function _ali_check_login(){
        $code=$this->input->get('auth_code', TRUE);
        $scope = 'auth_user';//需要授权
        $state = "";
        $this->load->config('ali_config');
        $user=$this->_getrow('user','id',['weixin_openid'=>$_COOKIE['openid']]);
        if(!$code && !$user){
            $REDIRECT_URI = urlencode(sprintf('%s%s', base_url(), $_SERVER['REQUEST_URI']));
            $url = 'https://openauth.alipay.com/oauth2/publicAppAuthorize.htm?app_id='.$this->config->item('al_appid').'&scope='.$scope.'&redirect_uri='.$REDIRECT_URI.'&state='.$state;
        ...
        if($user_id){
            $this->input->set_cookie('openid', $user_id, time()+3600*24);
            /**判断用户是否存在**/
            $row = $this->_getfield('user','id','weixin_openid='.$user_id .' and type=1');
            if(!$row){
                /**不存在**/
                log_message("error",json_encode($result));
                if($result){
                    $userData['type'] =1;   //支付宝用户
                    $userData['status'] =1;
                    $userData['createtime'] =time();
                    $userData['weixin_openid'] = $result->user_id;  //openid标识
                    if(!empty($result->avatar)) $userData['avatar'] = $result->avatar;  //支付宝头像
                    if(!empty($result->nick_name)) $userData['nickname'] = $result->nick_name; //支付宝名称
                    //$userData['city'] =$result->province . $result->city;      //城市(xxx省/xx市)
                    if($result->gender =='F'){
                        $userData['sex'] = 2;       //F 女， M 男
                    }elseif($result->gender =='M'){
                        $userData['sex'] = 1;
                    }
                    $re = $this->db->insert('user',$userData);
                    // $indexurl=base_url('h6/html/package.html?macno='.$macno);
    ...
    private function getAccessToken($code='',$refresh_token=''){
        $this->load->config('ali_config');
        ...
        $aop->postCharset='UTF-8';
        $aop->format='json';
        $this->load->library('CI_AliRequest');
        $request = $this->ci_alirequest;
    ...
    public function getUserInfo($accessToken){
        $this->load->config('ali_config');
        require_once './app/libraries/Alipay/AopSdk.php';
        $aops = new AopClient ();
        $aops->gatewayUrl = 'https://openapi.alipay.com/gateway.do';
        $aops->appId = $this->config->item('al_appid');
        $aops->rsaPrivateKey = $this->config->item('al_PrivateKey');
        $aops->alipayrsaPublicKey=$this->config->item('al_PublicKey');
        $aops->apiVersion = '1.0';
        $aops->signType = 'RSA2';
        $aops->postCharset='UTF-8';
        $aops->format='json';
        $request = new AlipayUserInfoShareRequest ();
        $result = $aops->execute ( $request , $accessToken );
...
class Admin_Controller extends SM_Controller { 
    ...
    public function __construct() {
        ...
        $this->_check_login();
        $this->_check_permit();
        $this->load->model('log_mdl');//后台操作日志模板
		$gid = $this->_admin->gid;
		$type = 0;
		if(in_array($gid, $this->_role[1]))$type = 1;
		if(in_array($gid, $this->_role[2]))$type = 2;
		if(in_array($gid, $this->_role[3]))$type = 3;
		$this->_admin->_role_type = $type;
		$this->assign('_admin', $this->_admin);
    }
       /**
     * 检查用户是否登录
     *
	...
	protected function _check_login()
	{
		...
		{
			$this->_admin = $this->admin_mdl->get_full_admin_by_username($this->session->userdata('aid'), 'id');
	...
	protected function _check_permit()
	{
		...
		else
		{
			 ...
			 {
			 	if($arr[1]=='admin_system'&&$arr[2]=='home')
			 	{
			 ...
			 if(strpos($url, '/admin_system') === 0)return true;
			 if(strrpos($url, '/searchaccount') !== false)return true;
			 if(strrpos($url, '/ueditor') !== false)return true;
			 if(strrpos($url, '/get_corp_shop') !== false)return true;
			 if(strrpos($url, '/get_percen') !== false)return true;
            if(strpos($url, 'qrcode') !== false)return true;
			 $result = $this->command_mdl->count_role_command($this->_admin->gid,$url);
			 if($result==0)
			 {
	...
	public function get_top_menus()
	{
		...
		$roles =$this->_admin->id==1?$this->command_mdl->get_commands_by_parent_id(0,1):$this->command_mdl->get_role_command_by_role_id($this->_admin->gid);
		foreach($roles as $v)
		{
				$this->top_menus.='<li><a href="'.$v['link_page'].'">'.$v['title'].'</a></li>';
			 
		}
	}
	
	
	public function get_left_menus($parent_id)
	{
		 
		$roles =$this->_admin->id==1?$this->command_mdl->get_commands_by_parent_id($parent_id,1):$this->command_mdl->get_role_command_by_role_id($this->_admin->gid,$parent_id);
		foreach($roles as $v)
		{
			$this->left_menus.='<li><a href="'.$v['link_page'].'" target="main">'.$v['title'].'</a></li>';
		}
	}
