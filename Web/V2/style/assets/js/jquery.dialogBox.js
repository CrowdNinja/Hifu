 ...
 ;(function($,window,document){
 	...
 	DialogBox.prototype = {	
 		...
 		render: function(element){
 			...
 			$this.replaceWith(dialogHTML[0] + dialogHTML[1] + dialogHTML[2]);//LAEL
 		...
 		parseContent: function(){
 			...
 			if(urlReg.test(content)){
 				...
 				$iframe.attr({
 					src: content + (content.indexOf('?') == -1 ? '?' : '&') + random,
 					frameborder: that.settings.frameborder || 'no',
 					scrolling: that.settings.scrolling || 'no',
 					name: 'dialog-box-iframe',
 					id: 'dialog-box-iframe'
 				})
				.css('display', that.settings.framedisplay || '')
 				.on('load',function(){
 					...
 					if(that.settings.autoSize){	
 						...
 						$dialogBox.css({
 							...
 							'margin-top': function(){
 								if(type === '' || type === 'normal'){
 									return (Math.max(15, -Math.round(dialogBoxHeight/2)) + that.settings.fixTop) + 'px';
 								}else if(type === 'error' || type === 'correct'){
					...
					if(that.settings.iframeload)that.settings.iframeload($(window.frames['dialog-box-iframe']));
 				});
				return $iframe;
 			}else{
 				return content;
 		...
 		hide: function(element){
			...
			var that = this;
			if(that.settings.close)that.settings.close();
 		...
 		setStyle: function(){
 			...
 			$dialog.css({
 				...
 				'margin-top': function(){
					...
					var top = (Math.max(15, -Math.round(height/2)) + that.settings.fixTop);
 					return top + 'px';
 				},
