(function (global, factory) {
    ...
    */
    ...
    function Chart(vido, props = {}) {
        ...
        return templateProps => wrapper(html `
        <div class=${className} test data-actions=${actions}>
        ...
        `;
          ...
          `;
                    ...
                    // scroll day from now on
                        ...
                        for (let i = 0; i < 24; i++) {
                            const left = start.add(i , 'hours');
                            const width = (start.add((i + 1), 'hours').valueOf() - left.valueOf()) / time.timePerPixel;
                            hours.push({
                                width,
                                formatted: left.format('HH:mm')
                            });
                        ...
                        htmlFormatted = html `
                            ...
                            ${hours.map(hour => html `
                                    ...
                                    >
                                        ${hour.formatted}
                                        <div class='hour_minutes_container'>
                                        ${[0,1,2,3].map(value => html `
                                            <div class=${className + '-content_child custom-minute'}>
                                            </div>
                                        `)}
                                        </div>
                                    </div>
          ...
          `;
                        ...
                        const start = dateMod.startOf('day');
                        for (let i = 0; i < 24; i++) {
                            const left = start.add(i, 'hour');
                            const width = (start.add((i + 1), 'hour').valueOf() - left.valueOf()) / time.timePerPixel;
                            hours.push({
                                width,
                                formatted: left.format('HH:mm')
                            });
  ...
  `;
    ...
    class TimeApi {
        ...
        recalculateFromTo(time) {
            ...
            time.leftGlobal = this.date(time.from)
            .startOf('day').add(9, 'hours').valueOf();
            return time;
        }
