 ...
 * @link         
 */
class Corptree_mdl extends DLC_Model
{
	...
	function search_row($keyword = '', $ids = false, $fields = false){
		...
		if(is_numeric($keyword)){
			$where = "gid in (".implode(",", $this->_role[1]).") and id='".$keyword."'";
			if($ids)$where .= " and id in ($ids)";
			$row = $this->_getrow('admin', '*', $where);
		}
		if(!$row){
			$where = "gid in (".implode(",", $this->_role[1]).") and (adminname='".$keyword."' or nickname='".$keyword."')";
			if($ids)$where .= " and id in ($ids)";
			$row = $this->_getrow('admin', '*', $where);
		}
		if($row){
	...
	function get_row($id, $ids = false, $fields = false){
		if(is_array($ids))$ids = implode(',', $ids);
		$where = "gid in (".implode(",", $this->_role[1]).") and id=$id";
		if($ids)$where .= " and id in ($ids)";
		$row = $this->_getrow('admin', '*', $where);
		if($row){
			//$row['_child'] = implode(',', $this->_getcol('admin', 'id', "pid=".$row['id']));
