...
class WechatPay {  
    ...
    public function getPrepayId($body,$out_trade_no,$total_fee,$notify_url,$openid) {  
        ...
        if ($result["return_code"] == "SUCCESS" && $result["result_code"] == "SUCCESS" && $result["prepay_id"]) {  
    ...
    public function getAppPrePayId($body,$out_trade_no,$total_fee,$notify_url){  
        $data = array();  
        $data["nonce_str"]    = $this->get_nonce_string();  
        $data["body"]         = $body;  
        $data["out_trade_no"] = $out_trade_no;  
        ...
        $data["spbill_create_ip"] = $_SERVER["SERVER_ADDR"];  
        $data["notify_url"]   = $notify_url;  
        $data["trade_type"]   = self::TRADETYPE_APP;  
        $result = $this->unifiedOrder($data);  
        if ($result["return_code"] == "SUCCESS" && $result["result_code"] == "SUCCESS" && $result["prepay_id"]) {  
            return $result["prepay_id"];  
        } else {  
     ...
     * @return array 
     */  
    public function refund($out_trade_no,$out_refund_no,$total_fee,$refund_fee,$op_user_id,$type=0){  
        $data = array();  
        $data["appid"] = $this->_config["appid"];  
        $data["mch_id"] = $this->_config["mch_id"];  
        $data["nonce_str"] = $this->get_nonce_string();  
        ...
        $data['refund_account']=$type?"REFUND_SOURCE_RECHARGE_FUNDS":"REFUND_SOURCE_UNSETTLED_FUNDS";  
     ...
     * @return array 
     */  
    public function refundByTransId($transaction_id,$out_refund_no,$total_fee,$refund_fee,$op_user_id,$type=0){  
        $data = array();  
        $data["appid"] = $this->_config["appid"];  
        $data["mch_id"] = $this->_config["mch_id"];  
        $data["nonce_str"] = $this->get_nonce_string();  
     ...
     * 获取app支付使用的第二个参数 
     */  
    public function get_app_package($prepay_id) {  
        $data = array();  
        $data["appid"] = $this->_config["appid"];  
